...
# =============================================================================
# API ENDPOINTS - CHECKING ACCOUNTS
# =============================================================================

@app.route('/api/checking-accounts', methods=['GET'])
def get_checking_accounts():
    """Get all checking accounts"""
    try:
        accounts = tracker.get_checking_accounts()
        accounts_list = []
        ...
        return jsonify({'success': True, 'data': accounts_list})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/checking-accounts', methods=['POST'])
def add_checking_account():
    """Add a new checking account"""
    try:
        data = request.get_json()
        
        required_fields = ['name', 'current_balance']
        for field in required_fields:
            if field not in data:
                return jsonify({'success': False, 'error': f'Missing required field: {field}'}), 400
        
        success = tracker.add_checking_account(
            name=data['name'],
        ...
        if success:
            return jsonify({'success': True, 'message': f'Added checking account: {data["name"]}'})
        else:
            return jsonify({'success': False, 'error': 'Failed to add checking account'})
            
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})

@app.route('/api/checking-accounts/<account_name>', methods=['PUT'])
def update_checking_account(account_name):
    """Update a checking account"""
    try:
        data = request.get_json()
        
        success = tracker.update_checking_account(account_name, **data)
        
        if success:
            return jsonify({'success': True, 'message': f'Updated checking account: {account_name}'})
        else:
            return jsonify({'success': False, 'error': f'Checking account "{account_name}" not found'})
            
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})

@app.route('/api/checking-accounts/<account_name>', methods=['DELETE'])
def remove_checking_account(account_name):
    """Remove a checking account"""
    try:
        success = tracker.remove_checking_account(account_name)
        
        if success:
            return jsonify({'success': True, 'message': f'Removed checking account: {account_name}'})
        else:
            return jsonify({'success': False, 'error': f'Checking account "{account_name}" not found'})
...
# =============================================================================
# API ENDPOINTS - RECURRING TRANSACTIONS
# =============================================================================
...
def get_recurring_transactions():
    ...
    try:
        transactions = tracker.get_recurring_transactions()
        transactions_list = []
        ...
        for trans_id, trans in transactions.items():
            transactions_list.append({
                ...
                'day_of_month': trans.get('day_of_month'),
                'is_active': trans.get('is_active', True),
                'created_date': trans.get('created_date', ''),
                'last_updated': trans.get('last_updated', '')
            })
...
def add_recurring_transaction():
    ...
    try:
        ...
        required_fields = ['name', 'amount', 'transaction_type', 'recurrence_pattern', 'start_date']
        for field in required_fields:
            if field not in data:
                return jsonify({'success': False, 'error': f'Missing required field: {field}'}), 400
        
        transaction_id = tracker.add_recurring_transaction(
            name=data['name'],
            amount=data['amount'],
        ...
        return jsonify({'success': True, 'message': f'Added recurring transaction: {data["name"]}', 'id': transaction_id})
        
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})

@app.route('/api/recurring-transactions/<transaction_id>', methods=['PUT'])
def update_recurring_transaction(transaction_id):
    """Update a recurring transaction"""
    try:
        data = request.get_json()
        
        success = tracker.update_recurring_transaction(transaction_id, **data)
        
        if success:
            return jsonify({'success': True, 'message': f'Updated recurring transaction'})
        else:
            return jsonify({'success': False, 'error': f'Recurring transaction not found'})
            
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})

@app.route('/api/recurring-transactions/<transaction_id>/deactivate', methods=['POST'])
def deactivate_recurring_transaction(transaction_id):
    """Deactivate a recurring transaction"""
    try:
        success = tracker.deactivate_recurring_transaction(transaction_id)
        
        if success:
            return jsonify({'success': True, 'message': f'Deactivated recurring transaction'})
        else:
            return jsonify({'success': False, 'error': f'Recurring transaction not found'})
...
@app.route('/api/forecast/<account_name>', methods=['GET'])
def generate_forecast(account_name):
    """Generate 12-month balance forecast for checking account"""
    try:
        months = request.args.get('months', 12, type=int)
        
        forecast_data = tracker.generate_account_forecast(account_name, months)
...
# =============================================================================
# API ENDPOINTS - CREDIT CARD PAYMENTS
# =============================================================================
...
def pay_credit_card():
    ...
    try:
        ...
        required_fields = ['checking_account', 'credit_card', 'amount']
        for field in required_fields:
            if field not in data:
                return jsonify({'success': False, 'error': f'Missing required field: {field}'}), 400
        
        result = tracker.pay_credit_card_from_checking(
            checking_account_name=data['checking_account'],
            credit_card_name=data['credit_card'],
            amount=data['amount']
        )
        
        return jsonify({
            'success': True,
            'message': f'Paid ${data["amount"]:,.2f} from {data["checking_account"]} to {data["credit_card"]}',
            'data': result
        })
...
@app.route('/api/upload-checking-transactions', methods=['POST'])
def upload_checking_transactions():
    """Upload and process checking account transactions from CSV"""
    try:
        ...
        if not account_name:
            return jsonify({'success': False, 'error': 'No account name provided'}), 400
        ...
        df = tracker._process_csv_file(filepath)
        
        if df.empty:
            return jsonify({'success': False, 'error': 'No valid transaction data found'})
        ...
        return jsonify({
            'success': True,
            'message': f'Processed {len(df)} transactions for {account_name}',
            'data': {
